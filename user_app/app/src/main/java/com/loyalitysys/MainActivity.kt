...
enum class DataStatus(val status: String) {
    ...
    PURCHASE_SCAN_ERROR_INVALID_QR("Purchase Data: Error, invalid QR")
}
        ...
        registerForActivityResult(ScanContract()) { result ->
            if (result.contents != null) {
                if (currentType == Types.CUSTOMER) {
                    ...
                    val parsedQRData = QRDataParser.parseQRCode(result.contents)
                    if (!parsedQRData.isEmpty) {
                        purchaseData = parsedQRData.get().sum.toString()
                        purchaseDataTextView.text = DataStatus.PURCHASE_SCANNED.status
                    } else {
                        purchaseData = ""
                        purchaseDataTextView.text = DataStatus.PURCHASE_SCAN_ERROR_INVALID_QR.status
                    }
